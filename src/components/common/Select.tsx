...
const Select = <T extends FieldValues>({ label, control, name, containerClassName, className, labelClassName, ...props }: SelectProps<T>) => {
  return (
    <React.Fragment>
      {control && name ? (
        <FormField
          ...
          render={({ field }) => (
            <FormItem className={cn("gap-0", containerClassName)}>
              ...
              <FormControl className={cn("my-0", className)}>
                <SelectComponent className={cn("cursor-pointer", className)} {...props} {...field} />
              </FormControl>
        ...
        <React.Fragment>
          {label && <Label className={cn("mb-1.5 px-0.5 text-sm font-medium", labelClassName)}>{label}</Label>}
...
const SelectComponent: React.FC<SelectProps<FieldValues>> = ({ options, placeholder, onChange, className, ...props }) => {
